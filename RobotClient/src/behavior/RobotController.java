...
public class RobotController implements Runnable {
	...
	public void getView() {
		...
		do {
			...
			if (Robot.getInstance().getBackX() == 0 || Robot.getInstance().getFrontX() == 0
					|| Robot.getInstance().getBackY() == 0 || Robot.getInstance().getFrontY() == 0) {
				System.err.println("Couldn't find robot");
				robotFound = false;
			} else {

				System.out.println(
						"Robot Cords: " + Robot.getInstance().getFrontX() + " " + Robot.getInstance().getFrontY() + " "
								+ Robot.getInstance().getBackX() + " " + Robot.getInstance().getBackY());
				robotFound = true;
			}
	...
	public void run() {
		...
		do {
			...
			iter++;
			System.out.println("Robotten er i " + cs.robot.get(0).getX() + " " + cs.robot.get(0).getY() + " og back "
					+ cs.robot.get(1).getX() + " " + cs.robot.get(1).getY());
			dto.clearData();
			dto.setRotation((float) dir);
			dao.sendData(dto);
